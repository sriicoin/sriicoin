# bash programmable completion for sriicoin-cli(1)
# Copyright (c) 2012-present The Sriicoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $sriicoin-cli for RPC
_sriicoin_rpc() {
...
_sriicoin_cli() {
    ...
    # save and use original argument to invoke sriicoin-cli for -help, help and RPC
    # as sriicoin-cli might not be in $PATH
    sriicoin_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($sriicoin_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_sriicoin_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _sriicoin_cli sriicoin-cli
