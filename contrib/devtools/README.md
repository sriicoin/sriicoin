...
gen-sriicoin-conf.sh
===================

Generates a sriicoin.conf file in `share/examples/` by parsing the output from `sriicoind --help`. This script is run during the
release process to include a sriicoin.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-sriicoin-conf.sh
```
