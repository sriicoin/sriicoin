...
be used in Sriicoin Core with the `-asmap` option.
...
to binary before being used with Sriicoin Core.
...
the `sriicoin-cli getnodeaddresses` command.
...
```
sriicoin-cli getnodeaddresses 0 > addrs.json
```
