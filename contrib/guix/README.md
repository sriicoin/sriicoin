# Bootstrappable Sriicoin Core Builds

This directory contains the files necessary to perform bootstrappable Sriicoin
Core builds.
...
To build Sriicoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Sriicoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/sriicoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/sriicoin-core/guix.sigs).
